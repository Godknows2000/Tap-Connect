...
class ProfileScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final displayUserId = userId ?? kUser.value?.uid;
    ...
    return FutureBuilder<Map<String, dynamic>?>(
      future: _fetchUserData(displayUserId),
      builder: (context, snapshot) {
        ...
        final email = userId == null
            ? kUser.value?.email ?? 'Unknown'
            : userData['email'] ?? 'Unknown';
        final isOwnProfile = displayUserId == kUser.value?.uid;
        ...
        return Scaffold(
          ...
          body: SingleChildScrollView(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                // Verify Email Section (only for own profile)
                if (isOwnProfile)
                  Padding(
                    padding: const EdgeInsets.symmetric(
